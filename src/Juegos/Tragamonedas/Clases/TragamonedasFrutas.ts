...
export class TragamonedasFrutas extends Tragamonedas {
    ...
    constructor(pCasino:Casino) {
        ...
        super(pCasino, "Tragamonedas de Frutas", simbol);     // Llama al constructor de la clase base
    ...
    jugar():void {
        ...
        this.matriz = auxMatriz; // Guardamos la matriz generada
    ...
    private calcularPremioPorFila(fila:string[]):number {
        ...
        for (let i = 1; i < fila.length; i++) {              // Recorremos la fila 
            if (fila[i] === fila[i - 1]) {                   // Si el símbolo actual es igual al anterior
                contador++;                                  // Incrementamos el contador
                if (contador > maxRepetidos) {               // Si el contador es mayor al maximo de repetidos
                    maxRepetidos = contador;                 // Actualizamos el máximo de símbolos iguales
                };
            } else {                                         // Si el símbolo actual es diferente al anterior
    ...
    mostrarMatriz():void {
        ...
        if (this.matriz && this.matriz[0] && this.matriz[0].length > 0) {
            ...
            for (let i = 0; i < this.matriz.length; i++) {
                ...
                let auxPremio : number = 0;

                // Obtenemos el premio para esta fila si this.premios existe
                if (this.premios) {
                    auxPremio = this.premios[i];
                };
